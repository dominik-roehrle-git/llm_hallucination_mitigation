...
if __name__ == "__main__":
    ...
    popularity_balanced = True
    train_injections = True

    for dataset in datasets:
        for layer in layers:
            print(f"Training probes for dataset: {dataset}, layer: {layer}")
            for probe_method in ["mini_fact"]:
                input_folder = f"train"
                model_output_path = "probes"  
                if popularity_balanced:
                    if train_injections:
                        probe_dataset_names = [f"{input_folder}/train_with_popularity_balanced_train_injections", f"{input_folder}/dev_with_popularity_balanced"]
                    else:
                        probe_dataset_names = [f"{input_folder}/train_with_popularity_balanced", f"{input_folder}/dev_with_popularity_balanced"]
                else:
                    probe_dataset_names = [f"{input_folder}/train_with_popularity_unbalanced", f"{input_folder}/dev"]
                ...
                if global_best_model is not None:
                    ...
                    if save_probes:
                        try:
                            ...
                            if popularity_balanced:
                                if train_injections:
                                    model_save_name = f"{model_output_path}/with_train_popularity_balanced_train_injections.pth"
                                else:
                                    model_save_name = f"{model_output_path}/with_train_popularity_balanced.pth"
                            else:
                                model_save_name = f"{model_output_path}/with_train_popularity_unbalanced.pth"
